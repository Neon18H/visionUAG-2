...
# Variable para controlar si el sistema de voz está activo
voice_active = False
...
def listen_for_commands():
    ...
    with sr.Microphone() as source:
        ...
        try:
            ...
            if "hello" in command:
                voice_active = True
                engine.say("Activando el sistema de voz")
                engine.runAndWait()
            elif "stop" in command:
                voice_active = False
                engine.say("Desactivando el sistema de voz")
                engine.runAndWait()
        except sr.UnknownValueError:
            pass  # No entendió nada, sigue escuchando
        except sr.RequestError:
            print("No se pudo obtener la respuesta de Google")

while True:
    # Llamar a la función para escuchar comandos (esto se puede hacer en otro hilo o dentro del loop)
    listen_for_commands()
    ...
    for i in range(len(boxes)):
        ...
        if confidence > 0.5:  # Umbral de confianza
            ...
            # Si el sistema de voz está activo y el objeto no ha sido detectado previamente o han pasado 15 segundos
            current_time = time.time()
            if voice_active and (object_name not in detected_objects or (current_time - detected_objects[object_name]['time']) >= 15):
